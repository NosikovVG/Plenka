...
export async function search(query, limit) {
  ...
  const qVec = await _embed(q);
   console.log('[AI-search] Запрос:', q);
console.log('[AI-search] Записей в индексе:', entries.length);
console.log('[AI-search] Топ-5 сходств:', scored.slice(0, 5).map(s => s.score.toFixed(3)));
